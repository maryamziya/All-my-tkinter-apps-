...
def update_volume_label(new_value):
...
#       * *Hint:* Link this function to the **`Scale`** using its **`command`** option.
    vol_num.config(text=f"Volume-{new_value}")
    vol_num.pack()
...
enable_notifications = Checkbutton(variable=notifications_on,onvalue=1,offvalue=0,text="Enable notifications")
enable_notifications.pack()
#       * *Hint:* You can use the `select()` method on the checkbutton after creation to set it to 'on' by default.
enable_notifications.select()
...
scale = Scale(from_=0,to=100,orient=HORIZONTAL,variable=volume_level,command=update_volume_label)
scale.set(50)
scale.pack()
# 3.  **Volume Label:** Create a **`Label`** that will display the current volume number. Start its text as "Volume: 50".
vol_num = Label(text="Volume-50")
vol_num.pack()
# 4.  **Save Button:** Create a **`Button`** labeled "Save Settings."
#
button = Button(text="Save Settings")
#

print(type(volume_level))
